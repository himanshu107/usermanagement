(function() {
  ...
  var tmpl = [
    ...
    '    <input class="hourInput" type="range" min="0" max="23" ng-model="inputHour" ng-change="updateNgModel()" />',
    '    <input class="minutesInput" type="range" min="0" max="59" ng-model="inputMinute"  ng-change="updateNgModel()"/> ',
    '    <input class="minutesInput" type="range" min="0" max="59" ng-model="inputSecond"  ng-change="updateNgModel()"/> ',
  ...
  var datetimePickerPopup = function($locale, dateFilter){
    ...
    var linkFunc = function(scope, element, attrs, ctrl) { //jshint ignore:line
      ...
      scope.updateNgModel = function(day) {
        ...
        scope.selectedDate = new Date(
          scope.mv.year, scope.mv.month, day, scope.inputHour, scope.inputMinute, scope.inputSecond, 0
        );
        scope.selectedDay = scope.selectedDate.getDate();
        if (attrs.ngModel) {
